...
SETTINGS = db.load('settings.json', {
    ...
    "payments": {},
    "categories": {} # {cid: {"name": "SG", "has_protocol": True}}
})
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('cat_'):
        ...
        await query.edit_message_caption(caption=f"📦 <b>{SETTINGS['categories'].get(cid, {}).get('name', 'Items')}</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
        ...
        if p:
            ...
            # Protocol ရွေးခိုင်းမလား စစ်သည်
...
async def add_cat_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await query.message.reply_text(f"✅ Category {context.user_data['temp_cat_name']} ထည့်သွင်းပြီးပါပြီ။", reply_markup=admin_home_menu())
    return ConversationHandler.END

# ... (add_p, add_pay, welcome_edit functions are standard) ...
