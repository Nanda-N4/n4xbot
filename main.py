...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- User Flow: Protocol Choice ---
        ...
        await query.edit_message_caption(caption=f"💳 <b>ငွေလွှဲမည့် အကောင့်ရွေးပါ</b>\n(ရွေးချယ်ထားသော Protocol: {proto.upper()})", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')

    # --- User Flow: Direct Payment Selection ---
    ...
    elif data == 'adm_back_home' or data == 'adm_close':
        if data == 'adm_close': await query.message.delete()
        else: await query.edit_message_text("🛠 <b>Admin Panel</b>", reply_markup=admin_home_menu(), parse_mode='HTML')
...
async def add_cat_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await query.message.reply_text(f"✅ {context.user_data['temp_cat_name']} ထည့်သွင်းပြီးပါပြီ။", reply_markup=admin_home_menu())
