...
async def main_menu_markup():
    ...
    for cid, _ in SETTINGS['categories'].items():
        keyboard.append([InlineKeyboardButton(get_cat_name(cid), callback_data=f"cat_{cid}")])
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Admin View Actions ---
        ...
        for cid, _ in SETTINGS['categories'].items():
            keyboard.append([InlineKeyboardButton(f"🗑 {get_cat_name(cid)}", callback_data=f"dcat_{cid}")])
        ...
        await query.edit_message_text("✅ Category ဖျက်ပြီးပါပြီ။", reply_markup=await admin_home_markup())
        ...
        await query.edit_message_text("🛠 <b>Admin Panel</b>", reply_markup=await admin_home_markup(), parse_mode='HTML')
...
async def add_pay_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await update.message.reply_text("✅ Payment Added!", reply_markup=await admin_home_markup())
    return ConversationHandler.END
