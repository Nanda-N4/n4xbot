...
# --- Keyboards (Defining as functions for reuse) ---
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(img):
        await update.message.reply_photo(photo=open(img, 'rb'), caption=welcome_text, reply_markup=main_menu_markup(), parse_mode='HTML')
    else:
        await update.message.reply_text(welcome_text, reply_markup=main_menu_markup(), parse_mode='HTML')

# --- Callback Handler (The Core Navigation) ---
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('cat_'):
        ...
        cat_name = SETTINGS['categories'].get(cid, {}).get('name', "ပစ္စည်းများ")
        ...
        if p:
            ...
            # Protocol လိုအပ်သော Category ဖြစ်ပါက Protocol ရွေးခိုင်းမည်
            if cat_info.get('has_protocol'):
                ...
                # Protocol မလိုပါက တိုက်ရိုက် Payment ဆီသွားမည်
    ...
    # --- Admin View Actions ---
        ...
        await query.edit_message_text("✅ Category ဖျက်ပြီးပါပြီ။", reply_markup=admin_home_markup())

    elif data == 'adm_manage_p':
        keyboard = []
        for pid, p in PRODUCTS.items():
            keyboard.append([InlineKeyboardButton(f"🗑 {p['name']}", callback_data=f"dlp_{pid}")])
        keyboard.append([InlineKeyboardButton("🔙 Back", callback_data='adm_back_home')])
        ...
        await query.edit_message_text("✅ Deleted!", reply_markup=admin_home_markup())
        ...
        await query.edit_message_text("🛠 <b>Admin Panel</b>", reply_markup=admin_home_markup(), parse_mode='HTML')

    elif data == 'adm_close':
        await query.message.delete()

    elif data == 'back_main':
        await query.edit_message_caption(caption=SETTINGS['welcome_text'], reply_markup=main_menu_markup(), parse_mode='HTML')

# --- Admin Conversations ---

# 1. Add Category
async def add_cat_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.message.reply_text("📂 <b>Category အမည် ပို့ပေးပါ:</b>", parse_mode='HTML')
    return A_CAT_NAME
...
async def add_cat_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await query.message.reply_text(f"✅ Category {context.user_data['temp_cat_name']} ထည့်ပြီးပါပြီ။", reply_markup=admin_home_markup())
...
async def add_p_desc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = []
    for cid, cat in SETTINGS['categories'].items():
        keyboard.append([InlineKeyboardButton(cat['name'], callback_data=cid)])
...
async def add_p_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('products.json', PRODUCTS)
    await update.callback_query.message.reply_text("✅ Added!", reply_markup=admin_home_markup())
    return ConversationHandler.END
...
async def welcome_edit_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await update.message.reply_text("✅ Saved!", reply_markup=admin_home_markup())
    return ConversationHandler.END
...
async def add_pay_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await update.message.reply_text("✅ Payment Added!", reply_markup=admin_home_markup())
    return ConversationHandler.END
