...
# States
(A_P_NAME, A_P_PRICE, A_P_DESC, A_P_CAT, E_WELCOME, A_PAY_TYPE, A_PAY_INFO, A_CAT_NAME) = range(8)
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('cat_'):
        ...
        await query.edit_message_caption(caption=f"📦 <b>{SETTINGS['categories'].get(cid, 'Items')}</b>", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='HTML')
    ...
    # User Side: Final Show QR & Notify Button
    ...
    # User Side: Notify Admin
    elif data.startswith('notif_'):
        p_id = data.replace('notif_', '')
        p = PRODUCTS.get(p_id)
        user = query.from_user
        # Admin ဆီ အကြောင်းကြားစာ ပို့မည်
        admin_msg = f"🔔 <b>ငွေလွှဲအကြောင်းကြားစာ</b>\n\n👤 User: <a href='tg://user?id={user.id}'>{user.full_name}</a> (@{user.username})\n📦 ပစ္စည်း: <b>{p['name'] if p else 'Unknown'}</b>\n💰 စျေးနှုန်း: {p['price'] if p else '-'}\n\nပြေစာစစ်ဆေးပေးပါရန်။"
...
async def add_cat_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.callback_query.message.reply_text("📂 <b>Category အမည်အသစ် ပို့ပေးပါ:</b>", parse_mode='HTML')
    return A_CAT_NAME
...
async def add_cat_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    db.save('settings.json', SETTINGS)
    await update.message.reply_text(f"✅ {cat_name} ကို ထည့်သွင်းပြီးပါပြီ။", reply_markup=admin_home_menu())
    return ConversationHandler.END

# ... (ကျန်တဲ့ Admin Functions - add_p, add_pay, welcome_edit တွေက အဟောင်းအတိုင်းပဲ ထည့်ပေးရပါမယ်) ...
...
async def run_bot():
    ...
    admin_conv = ConversationHandler(
        ...
        states={
            ...
            A_P_CAT: [CallbackQueryHandler(add_p_final, pattern="^cat_")],
...
# (မှတ်ချက် - add_p_start, add_pay_start စတဲ့ function တွေ အပေါ်က version အတိုင်း ပြည့်စုံအောင် ပြန်ထည့်ပါ)
